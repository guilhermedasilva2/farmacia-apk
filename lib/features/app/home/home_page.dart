...
import 'package:meu_app_inicial/features/onboarding/infrastructure/services/consent_service.dart';
...
import 'package:meu_app_inicial/utils/product_shimmer.dart';
...
import 'package:meu_app_inicial/features/categories/domain/repositories/category_repository.dart';
import 'package:meu_app_inicial/features/categories/infrastructure/repositories/category_repository_impl.dart';
import 'package:meu_app_inicial/features/categories/infrastructure/local/categories_local_dao.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  CategoriesLocalDao? _categoriesLocalDao;
  ...
  Future<void> _initCategories() async {
    try {
      ...
      _categoriesLocalDao = CategoriesLocalDao(prefs);
      await _loadCategories();
    } catch (e) {
      debugPrint('Error initializing categories DAO: $e');
      // Fallback to minimal load
      await _loadCategories();
    }
  }
  ...
  Future<void> _loadCategories() async {
    ...
    if (_categoriesLocalDao != null) {
      try {
        final localDtos = await _categoriesLocalDao!.listAll();
        if (localDtos.isNotEmpty && mounted) {
          setState(() {
            _categories = localDtos.map((dto) => Category(
              id: dto.id,
              name: dto.name,
              slug: dto.slug ?? '',
            )).toList();
    ...
    // 2. Buscar remoto e atualizar cache
    try {
      ...
      if (_categoriesLocalDao != null) {
        ...
        await _categoriesLocalDao!.upsertAll(dtos);
      }

    } catch (e) {
      debugPrint('Error loading remote categories in HomeScreen: $e');
    }
  }
...
class _CategoryCarouselState extends State<_CategoryCarousel> {
  ...
  static const int _initialPage = 10000; // Página inicial no meio do "infinito"

  @override
  void initState() {
    super.initState();
    _pageController = PageController(
      viewportFraction: 0.45, // Mostra parte do próximo card
